...
- Project starts with a single `docker compose up` command and is serving on port 4000 within 90 seconds of a cold start
...
- As an evaluator, I want to start the API and PostgreSQL 16 with a single `docker compose up` so that I can review the project without installing Elixir locally
- As a backend developer, I want ConnCase, DataCase and ChannelCase test helpers with fixtures so that every later feature can be tested from day one
...
- As a new user, I want to register with a unique `@username`, display name, email and password so that I have an identity in the system
- As a user, I want to log in with my email and password and receive a bearer token so that I can call authenticated endpoints
...
- As the system, I want to hash passwords with bcrypt so that a database dump does not expose credentials
...
**Capabilities:**
- Phoenix 1.8.8 application in API-only mode with `elixir: "~> 1.20"` in `mix.exs`, served by Bandit on port 4000, with the browser/LiveView pipeline removed from the router; LiveDashboard remains available at `/dev/dashboard` in dev only
...
- `GET /api/health` returning 200 with `{"status": "ok", "database": "up"}` after executing `SELECT 1`, or 503 with `"database": "down"` when the Repo is unreachable
- Docker Compose with two services: `db` on `postgres:16-alpine` with a named volume and a healthcheck (`pg_isready`, 5 s interval, 10 retries), and `api` built from a multi-stage Dockerfile that waits for the healthcheck, runs `mix ecto.setup`, and starts the server on port 4000
...
- Test harness: `DataCase` and `ConnCase` with SQL sandbox, a `ChannelCase` for socket and channel tests, and a `Api.Fixtures` module exposing `user_fixture/1`, `contact_fixture/2`, `private_conversation_fixture/2`, `group_fixture/2` and `message_fixture/2`
- Quality gate: `mix precommit` runs `compile --warnings-as-errors`, `deps.unlock --unused`, `format` and `test`; `excoveralls` is configured with a minimum coverage threshold of 80% over `lib/api`

**Experience:** A reviewer clones the repository, copies `.env.example` to `.env`, and runs `docker compose up`. The `db` service starts and becomes healthy, the `api` service waits for it, runs migrations and seeds, and logs `Running ApiWeb.Endpoint with Bandit ... at 0.0.0.0:4000`. `curl localhost:4000/api/health` returns `{"status":"ok","database":"up"}`. A developer working without Docker runs `mix setup` against a local PostgreSQL 16 and gets the same result via `mix phx.server`. Every failing request, regardless of which controller produced it, returns the same envelope shape, so the client parses `errors.code` for branching and displays `errors.detail` directly.
...
**Provides:**
- User account records: user id, `@username`, display name, email, `last_seen_at` (used by F03, F10, F11)

**Core Scope:**
- Registration, login, current-user endpoint, JWT generation and verification, the `RequireAuth` plug, and socket authentication

**Full Scope additions:**
- Login rate limiting per IP and per email, and token revocation on logout

**Capabilities:**
- `users` table: `id` (UUID), `username` (citext, unique), `display_name`, `email` (citext, unique), `hashed_password`, `last_seen_at`, timestamps; unique indexes on `username` and `email`
...
- `display_name`: 2–60 characters, any printable characters, trimmed
- `password`: 8–72 characters, hashed with `bcrypt_elixir` at cost 12; never returned in any response
- `POST /api/auth/register` accepting `username`, `display_name`, `email`, `password`; returns 201 with `{"user": {...}, "token": "...", "expires_at": "..."}`
- `POST /api/auth/login` accepting `email` and `password`; returns 200 with the same shape
- `GET /api/auth/me` returning the authenticated user
- JWT: HS256 signed with `JWT_SECRET`, claims `sub` (user id), `exp` (7 days), `iat`, `jti`; verified on every request by the `RequireAuth` plug, which assigns `conn.assigns.current_user` and halts with 401 otherwise
- Socket authentication: `UserSocket.connect/3` reads the `token` connect param, verifies the same JWT, and assigns `socket.assigns.current_user_id`; returns `:error` on any failure, which closes the connection
- Login rate limit: 10 failed attempts per IP per 60 seconds and 5 per email address per 60 seconds, returning 429 with `Retry-After`
- Logout (`DELETE /api/auth/session`) adds the token `jti` to a revocation set held in an ETS-backed cache with a TTL equal to the token's remaining lifetime; revoked `jti` values are rejected by both the plug and the socket

**Experience:** A client posts registration data; on success it receives the user record and a bearer token in one response, so no second login round trip is needed. Every subsequent request carries `Authorization: Bearer <token>`, and the WebSocket is opened with the same token as a connect parameter. A username collision returns 422 with `fields: {"username": ["has already been taken"]}` so the client can highlight the field. Login with wrong credentials returns a single generic message regardless of whether the email exists, and the password hash comparison runs even for unknown emails so response time does not reveal account existence.

**Error Handling:**
- Invalid credentials: 401 with `{"errors": {"code": "invalid_credentials", "detail": "Invalid email or password"}}`, identical for unknown email and wrong password
- Duplicate username or email: 422 with `code: "validation_error"` and the offending field in `fields`
...
- `GET /api/contacts` returning all contacts sorted by `display_name` ascending using a case-insensitive, accent-insensitive collation, each entry containing contact id, user id, `@username` and display name; maximum 500 contacts per user
...
**Capabilities:**
- `ApiWeb.UserSocket` mounted at `/socket`, authenticated by the JWT passed as a connect param (F02); the socket id is `"user_socket:<user_id>"` so all of a user's sockets can be disconnected together
...
**Consumes:**
- F02: user account records (user id, `@username`, display name, email, `last_seen_at`)
...
- A "Design decisions" section in the README justifying the significant choices: JWT over cookie sessions for a cross-origin SPA, REST plus Channels over an all-channel or GraphQL API, a single `conversations` table for both private and group conversations, keyset over offset pagination, and unidirectional contacts
...
- Password reset, email verification, and any transactional email; Swoosh remains configured but unused
...
- Horizontal scaling and multi-node clustering; the deliverable targets a single node with PubSub PG2 configured but untuned, and DNSCluster left at its default
...
- **F01 API Foundation and Development Environment** — converts the Phoenix scaffold to an API-only application, establishes the UUID/timestamp schema conventions, the global JSON error envelope and fallback controller, CORS, the Docker Compose environment, and the ExUnit harness (DataCase, ConnCase, ChannelCase, fixtures) that every other feature's tests are written against
- **F02 User Account and JWT Authentication** — adds the users table and the cross-cutting authentication layer: the `RequireAuth` plug applied to every authenticated pipeline and the `UserSocket.connect/3` callback, which all later REST endpoints and channels rely on without declaring it
...
- [ ] `GET /api/health` returns 503 with `"database": "down"` when the Repo connection fails
...
- [ ] `docker compose up` on a clean machine brings the API to a serving state on port 4000 with migrations and seeds applied, verified by a successful `/api/health` call
- [ ] Booting without `JWT_SECRET` set aborts startup with a message naming the missing variable
- [ ] `mix precommit` passes and reported coverage over `lib/api` is at least 80%

### F02. User Account and JWT Authentication
- [ ] Registering with a valid username, display name, email and password returns 201 with the user record, a token and `expires_at`
- [ ] No response anywhere in the API includes `hashed_password` or `password`
- [ ] Registering with an existing username returns 422 with `fields.username` populated; registering with an existing email returns 422 with `fields.email` populated
- [ ] Registering with a username containing uppercase letters, spaces or fewer than 3 characters returns 422 and creates no user
- [ ] Usernames are treated case-insensitively: registering `AnaBeatriz` after `anabeatriz` exists is rejected
- [ ] Login with correct credentials returns 200 with a token whose `sub` claim equals the user id and whose `exp` is 7 days ahead
- [ ] Login with a wrong password and login with an unknown email both return 401 with the identical `invalid_credentials` body
- [ ] `GET /api/auth/me` returns the authenticated user with a valid token, and 401 with `code: "unauthenticated"` without one
- [ ] A token signed with a different secret returns 401 and never resolves to a user
- [ ] An expired token returns 401 with `code: "token_expired"`, distinct from the generic unauthenticated code
...
### F03. Contact Management
- [ ] Adding an existing user by `@username` returns 201 with the contact record including user id, `@username` and display name
- [ ] Adding by a username with a leading `@` and by the bare username both resolve to the same user
- [ ] Adding a non-existent username returns 404 with `code: "user_not_found"` and the searched username in `detail`
- [ ] Adding the same contact twice returns 409 with `code: "contact_already_exists"` and creates no second row
- [ ] Adding oneself returns 422 with `code: "self_contact"`
- [ ] `GET /api/contacts` returns only the caller's contacts, sorted by display name ascending, case- and accent-insensitively
- [ ] Adding a contact does not create any row in the target user's contact list
- [ ] `DELETE /api/contacts/:id` returns 204 and the contact no longer appears in the list
- [ ] Deleting a contact id that belongs to another user returns 404, not 403
...
- [ ] The README contains a "Design decisions" section justifying JWT, REST plus Channels, the single conversations table, keyset pagination and unidirectional contacts
...
- [ ] A user record created by registration (F02) is resolvable by `@username` in the add-contact flow (F03), and the returned contact carries that user's id and display name
